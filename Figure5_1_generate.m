%% RMSE confrontato con DEB degli stimatori NLoS ML e RML per valori diversi di campioni di luce non coerente K_1 (nel caso NLoS lo chiamiamo K_1), in funzione del SNR quando OIRS ha l'orientamento iniziale non ottimizzato
...
[alpha(1),beta(1)] = computeTilt(RIS(1,:),LED,PD_test,Norm(1,:));
...
for sample_index=1:length(test_samples)
    for test_snr=1:length(SNR_db)
        ...
        sum_ML=0;
        parfor i=1:times
            [noises, thermal_var] = noisesEstimation(R_pd*p*h0 + R_pd*p*hn, q_0, k_B, T_k, eta, I_2, I_3, Gamma, A_pd, g_m, I_bg, G_0, B, test_samples(sample_index)); % caso NLoS
            % [noises, thermal_var] = noisesEstimation(R_pd*p*h0, q_0, k_B, T_k, eta, I_2, I_3, Gamma, A_pd, g_m, I_bg, G_0, B, test_samples(sample_index)); % caso LoS
        ...
        CRLB(sample_index,test_snr)=sqrt(CRLB_NLoS(R_pd,p,h0,hn,d0_real,d1_real,r_1,m,alp,bet,test_samples(sample_index)));
    end
...
save("figure5_1.mat","CRLB","RMSE_ML","RMSE_RML","SNR_db","dist_r","dist_e");
elapsedtime5_1=toc
...
savings.Figure5_1_generate = elapsedtime5_1;  % aggiungi/aggiorna campo
