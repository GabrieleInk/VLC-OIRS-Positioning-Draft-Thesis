...
for sample_index=1:length(test_samples)
    for test_snr=1:length(SNR_db)
        ...
        sum_ML=0;
        parfor i=1:times
            [noises, thermal_var] = noisesEstimation(R_pd*p*h0 + R_pd*p*hn, q_0, k_B, T_k, eta, I_2, I_3, Gamma, A_pd, g_m, I_bg, G_0, B, test_samples(sample_index)); % caso NLoS distance estimation
            % [noises, thermal_var] = noisesEstimation(R_pd*p*h0, q_0, k_B, T_k, eta, I_2, I_3, Gamma, A_pd, g_m, I_bg, G_0, B, test_samples(sample_index)); % caso LoS distance estimation
...
save("figure5_2.mat","CRLB","RMSE_ML","RMSE_RML","SNR_db","dist_r","dist_e");
elapsedtime5_2=toc
...
savings.Figure5_2_generate = elapsedtime5_2;  % aggiungi/aggiorna campo
