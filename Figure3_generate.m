%% RMSE confrontato con DEB degli stimatori NLoS ML e RML per valori diversi di campioni di luce non coerente K_1 (nel caso NLoS lo chiamiamo K_1), in funzione dell'angolo azimutale, che descrive lo spostamento del PD lungo un arco di circonferenza attorno all'OIRS
...
test_samples=[5,10,20,40,80]; % valori di K campioni 

RMSE_RML=zeros(length(test_samples),length(angles));
...
dist_r=zeros(length(test_samples),length(angles));
dist_e=zeros(length(test_samples),length(angles));
...
for sample_index=1:length(test_samples)
    ...
    for test_angle=1:length(angles)
        angle=deg2rad(angles(test_angle));
        PD_test = [x_max/2 + 2.5*cos(angle), 2.5*sin(angle), 0];
        ...
        parfor i=1:times
            ...
            d1_r_v(test_angle,i)= abs(d1_real - d1_r);
            d1_e_v(test_angle,i)= abs(d1_real - d1_e); 
        end
        end
        RMSE_RML(sample_index,test_angle)= sqrt(sum_RML/times);
        RMSE_ML(sample_index,test_angle)= sqrt(sum_ML/times);
        CRLB(sample_index,test_angle)=sqrt(CRLB_NLoS(R_pd,p,h0,hn,d0_real,d1_real,r_1,m,alpha,beta,test_samples(sample_index)));
    end
