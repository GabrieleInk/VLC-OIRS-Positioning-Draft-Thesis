...
for sample_index=1:length(test_samples)
    ...
    for test_snr=1:length(SNR_db)
        ...
        parfor i=1:times
            ...
            d0_r_v(test_snr,i)= abs(d0_real - d0_r);
            d0_e_v(test_snr,i)= abs(d0_real - d0_e);
        end
    ...
    dist_e(sample_index,:)=mean(d0_e_v');
    dist_r(sample_index,:)=mean(d0_r_v');
end

save("figure1_1_b400.mat","CRLB","RMSE_ML","RMSE_RML","SNR_db","dist_r","dist_e");
